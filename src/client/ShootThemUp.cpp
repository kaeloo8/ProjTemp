...
void ShootThemUp::StartGame()
{
	...
	lObject = CreateEntity<Background>("Mur");
	lObject->SetScale(1, 1);
	lObject->SetPosition(0 + lObject->mWidth / 2, 0 + lObject->mHeight / 2);
	lObject->Layout = 2;
...
void ShootThemUp::UpdateWorld() {
	...
	lObject->SetPosition(cam.GetPosX() + lObject->mWidth / 2, cam.GetPosY() + lObject->mHeight / 2);
